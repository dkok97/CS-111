...
int main(int argc, char* argv[])
{
  	...
  	while ((ch = getopt_long(argc, argv, "p:s:l:di:h:", longopts, NULL)) != -1)
    {
      switch (ch)
      {
            ...
            case 'h':
              ...
              break;
          	default:
              fprintf(stderr, "Usage: lab4c_tls [--period=#] [--scale=[C/F]] [--log=FILE] [--host=#] [--id=#] PORTNUM\n");
              exit(1);
    	}
  }
  ...
  if(ssl_context == NULL)
  {
    cmd_err="SSL_CTX_new(TLSv1_client_method())";
		report_error_2(cmd_err);
  }
  ...
  while(1)
  {
    ...
    {
      int bytes_read = SSL_read(ssl, cmdBuf, strlen(cmdBuf));
      ...
      if (debug) fprintf(stderr, "READ %s %i\n", cmdBuf, bytes_read);
